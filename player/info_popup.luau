...
function InfoPopup.show(player)
    ...
    if Settings.friends.playerMutualFriends[player] then
        local mutuals = Settings.friends.playerMutualFriends[player]
        create_info_item("Mutual Friends", #mutuals, order) order = order + 1
    end
